...
import BoatTradeScreen from "./BoatTradeScreen";
...
export default function VertexLayer({
  ...
  const [boatPopup, setBoatPopup] = useState(null);
  ...
  function renderBoats() {
    ...
    return boatData.map((boat, boatIndex) => {
      ...
      if (row1 < 0 || row2 < 0 || col1 < 0 || col2 < 0) return null;
      if (row1 >= vertexData.length || row2 >= vertexData.length) return null;
      if (col1 >= vertexData[row1].length || col2 >= vertexData[row2].length)
        return null;

      const p1 = getVertexScreenPos(
        row1,
        col1,
        vertexData,
        xSpacing,
      ...
      const centerY =
        (vertexData.length * ((oddRowOffset + evenRowOffset) / 2)) / 2;
        ...
        >
          ...
          <Pressable
            onPress={() => {
              if (isPlayerTurn) {
                console.log(
                  `[BOAT] Clicked boat ${boatIndex} resourceType=${resourceType}`,
                );
                setBoatPopup({
                  x: boatX - BOAT_SIZE / 2,
                  y: boatY - BOAT_SIZE / 2,
    ...
    >
          ...
          >
            {row.map((value, colIndex) => {
              ...
              const dotColor = isVacant
                ...
                : playerColors[isCity ? value - 10 : value];

              const dotSize =
                isVacant && isPlayerTurn ? HEX_SIZE * 0.26 : HEX_SIZE * 0.22;
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    if (isPlayerTurn) {
                      const screenX =
                        rowOffset + colIndex * xSpacing + HEX_SIZE * 0.11;
                      const screenY = y + HEX_SIZE * 0.11;
                      console.log(
                        `[VERTEX] Clicked (${colIndex}, ${rowIndex / 2})`,
                      );
      ...
      {boatPopup && (
        <BoatTradeScreen
          ...
          y={boatPopup.y}
          boat={boatPopup.boat}
          resourceType={boatPopup.resourceType}
          onClose={() => setBoatPopup(null)}
          playerNumber={playerNumber}
          serverUrl={serverUrl}
          guid={guid}
        />
